...
# Model path can be overridden via the SCISPACY_MODEL_PATH environment variable,
# so this script stays portable across machines instead of being hardcoded to
# one Windows user's Downloads folder. Falls back to the original path if the
# env var isn't set, so behavior is unchanged unless you explicitly configure it.
SCISPACY_MODEL_PATH = os.environ.get(
    "SCISPACY_MODEL_PATH",
    r"C:\Users\0yash\Downloads\en_core_sci_sm-0.5.4.tar.gz\en_core_sci_sm-0.5.4\en_core_sci_sm",
)

nlp = spacy.load(SCISPACY_MODEL_PATH)
...
def extract_entities(text):
    ...
    for ent in doc.ents:
        ...
        if ent._.kb_ents:
            cui, score = ent._.kb_ents[0]  # FIX: was kb_ent (missing 's'); ent._.kb_ents is a sorted list of (cui, score)
...
# for detecting abbr. like BP etc
...
def classify_entities(entities):
    ...
    for entity in entities:
        ...
        if tuis & SYMPTOM_TUIS or surface in SYMPTOMS:
            ...
            metadata["conditions"].append(label)  # FIX: was metadata["condition"] (key mismatch with "conditions")
    ...
    for key in metadata:  # FIX: dedup loop was indented inside the per-entity loop; moved outside so it runs once after all entities processed
...
def chunk_page(text, chunk_size=500, chunk_overlap=50):
    ...
    while start < len(text):
        ...
        chunks.append(chunk)
        start = end - chunk_overlap  # FIX: start never advanced before, causing an infinite loop since chunk_overlap was defined but unused
...
def main():
    ...
    chunk_id = 0  # global counter, shared across PDF and CSV ingestion so ids stay unique

    pdf_texts = []      # FIX: these collector lists never existed before, so nothing was ever embedded/added for PDFs
    pdf_ids = []
    pdf_metadatas = []
    ...
    for doc in documents:
        ...
        for chunk in chunks:
            current_chunk_id = f"{chunk_id}"  # FIX: was set once outside all loops, so every chunk shared id "0"
            ...
            for entity in entities:
                if entity["cui"]:
                    medical_index["cuis"][entity["cui"]].append(current_chunk_id)  # FIX: was entity["cuis"] (typo, KeyError)

            for symptom in classified["symptoms"]:
                medical_index["symptoms"][symptom.lower()].append(current_chunk_id)

            for condition in classified["conditions"]:  # FIX: was classified["condition"] (KeyError)
            ...
            meta = {
                ...
                "type": "guidelines",
                "abbreviations": json.dumps(abbreviations) if abbreviations else ""  # FIX: was json.dump (writes to file, returns None) -> json.dumps
            }
            meta.update(entities_to_chromadb(entities, classified))  # FIX: was missing the call (no parens/args), so it tried to .update a function object

            pdf_texts.append(chunk)
            pdf_ids.append(current_chunk_id)
    ...
    if pdf_texts:
        ...
        add_in_batches(
            ...
            embeddings=pdf_embeddings.tolist()
        )
    ...
    for file in os.listdir(files_folder):
        ...
        csv_path = os.path.join(files_folder, file)  # FIX: was os.listdir.join (os.listdir is a function, not a module)
        ...
        for row_ids, row in df.iterrows():
            ...
            current_chunk_id = f"{chunk_id}"  # FIX: CSV rows never got a chunk id before, and ids reset to 0 per file causing collisions
            ...
            # ------------------------------------- indexing -----------------------------------------
            # FIX: CSV rows were never added to medical_index at all before - this whole block is new
            for entity in entities:
                if entity["cui"]:
            ...
            meta = {
                ...
                "focus_area": str(row["focus_area"]),  # FIX: key was named "classified" before, which was misleading (not the classification dict)
                "type": "qa",
                "file_name": file,
                "row_num": int(row_ids) + 1,
                "abbreviations": json.dumps(abbreviations) if abbreviations else ""  # FIX: json.dump -> json.dumps
            }
